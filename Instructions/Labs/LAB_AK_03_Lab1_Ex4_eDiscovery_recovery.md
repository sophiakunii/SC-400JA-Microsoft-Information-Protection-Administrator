...
4. 「**Records Management(レコード管理)**」ロールを検索して、選択します。

4. Record Managment ウィンドウで、「**メンバー**」カテゴリの横にある「**編集**」を選択します。
...
特定のメールを探すための新しいコンテンツ検索を正常に作成し、ユーザーのメールボックスからフィッシング メールを削除するために消去操作を行いました。消去操作は、組織の**管理者ロール**のメンバーとしてのみ行うことができ、コンプライアンス管理者はこれに該当しません。
