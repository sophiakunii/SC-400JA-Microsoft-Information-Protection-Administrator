...
この演習では DLP ポリシーを作成しますが、ポリシーが有効になるまで、約24時間が必要です。
...
5. 「**テンプレートの利用またはカスタム ポリシーの作成**」ページで「**カスタム**」、真ん中のウィンドウで「**カスタム ポリシー**」を選択します。「**次へ**」を選択しましょう。

6. 「**DLP ポリシーの名前を設定**」ページで、「*クレジット カードの DLP ポリシー*」を「**名前**」フィールドに、「*クレジット カード番号を共有しないようにする。*」を「**説明**」フィールドに入力します。「**次へ**」をクリックします。

7. 「**ポリシーを適用する場所の選択**」ページで **Teams chat and channel messages** オプションのみを有効にし、「**次へ**」を選択します。
...
14. 「**ルールの作成**」ページで「**+ 条件の追加**」を選択し、ドロップダウン メニューから 「**Microsoft 365 からコンテンツを共有されている**」を選択します。

15. 「**Microsoft 365 からコンテンツを共有されている**」セクションで「**組織内の連絡先のみ**」オプションを選択します（既定では選択されています）。

16. 「**アクション**」セクションで「**+ 条件を追加**」を選択し、「**Microsoft 365 の場所にあるコンテンツへのアクセスを制限またはコンテンツを暗号化する**」を選択します。

17. 「**Microsoft 365 の場所にあるコンテンツへのアクセスを制限またはコンテンツを暗号化する***」をチェックし、「**すべてのユーザーをブロックします**」を選択します。

18. **ユーザー通知**セクションで、スイッチを選択し、「**オン**」のポジションにします。

19. **Microsoft 365 サービス** セクションで、「**Office 365 サービスのユーザーにポリシーヒントの通知を表示する**」をチェックします。

19. **ユーザーによる上書き**セクションで、「**Microsoft 365 サービスによる上書きを許可します**」をチェックし、「**上書きするには業務上の理由が必要です**」オプションを選択します。

20. **インシデント レポート**セクションの「**管理者通知とレポートでこの重要度レベルを使用します**」ドロップダウンで「**低**」を選択します。

21. **インシデント レポート**セクションで、「**ルールが一致した場合に管理者にアラートを送信します。**」スイッチを選択します。既定の設定はポリシーを作成するユーザー（Jonis）に通知を行います。

22. 「**保存**」を選択し、「**次へ**」を選択します。

23. **テストを行うかポリシーを有効にする**ページで、「**最初にテストしてください**」を選択し、「**テスト モード中にポリシー ヒントを表示します**」を選択します。
...
6. 「**ポリシーを適用する場所の選択**」ページで、「**Exchange email**」オプションを有効にし、それから「**ポリシーの確認と作成**」ページが開くまで「**次へ**」を選択します。
...
4. 次のコマンドを PowerShell に入力し、すべての Exchange メールボックスで Driver's License をスキャンする DLP ポリシーを作成します。
...
6. 次のコマンドを用いて **Driver's License DLP Policy**を見直します。
...
1. 「**テストを行うかポリシーを有効にする**」ページが開くまで「**次へ**」を選択し、「**すぐに有効にします**」を選択します。
...
DLP ポリシーを 2 つ作成したところで、より制限の厳しいポリシーが、より制限の緩いポリシーより優先して処理されるようにします。ここでは、運転免許証のポリシーに、より高い優先順位を与えます。
...
5. 「**ファイル ポリシーの作成**」ページで、「**ポリシー名**」フィールドに「**ファイルのクレジット カード情報**」を、「**説明**」フィールドに「**クレジット カード番号がファイルで共有されることを防ぐ。**」を入力します。

6. 「**ポリシー重要度**」は「**低**」とし、「**カテゴリー**」が「**DLP**」に設定されていることを確認します。ファイル ポリシーでは、これが既定の設定です。

7. 「**このポリシーの対象となるファイルのためのフィルターを作成**」エリアで、ドロップダウン メニュー「**公開 (インターネット)、外部、公開**」を展開し、「**内部**」を追加します。
...
### タスク 8 - Power Automate による DLP ポリシーを作成する

会社では Power Automate のフローを用いて、データを SharePoint Online と SalesForce で共有します。このタスクでは、既存のフローを続行させつつ、「SharePoint Online」「Salesforce」 と「ビジネス以外のアプリ」として定義されたアプリとの間でデータを共有するフローが作成されるのを防止するための、PowerPlatform 向けの DLP ポリシーを作成します。
...
3. **Power Platform 管理センター**の、左側のナビゲーション ウィンドウで、**ポリシー** 配下の「**データ ポリシー**」を選択します。

4. 「**データ ポリシー**」ページで「**+ 新しいポリシー**」を選択します。

5. 「**ポリシーに名前をつける**」ページで、「**Tenant-wide SharePoint Policy**」と入力して、「**次へ**」を選択します。

6. 「**非ビジネス**」タブで「**SharePoint**」と「**Salesforce**」を選択し、ページ上部の「**ビジネスに移動する**」を選択します。
...
SharePoint Online と SalesForce でないすべてのコネクタを利用するフローをユーザーが作成することを防止する Power Platform の DLP ポリシーが作成されました。
