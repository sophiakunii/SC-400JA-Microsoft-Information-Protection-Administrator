...
4. 1 つ目のメール アイテムを右クリックし、メニューで「**Assign Policy（ポリシーの割り当て）**」を選択します。
...
3.  「**New Fodler...**」を選択し、次のように入力します。<br><br>
	
	- VAT の還付

4.  左側のパネルで新しく作成した **VATの還付**フォルダーを右クリックします。

5.  メニューから「**Properties**」を選択します

6.  「**Policy**」タブを選択します

7.  「**Folder Policy**」ドロップダウン リストで、「**VAT の還付と関連書類**」に設定します。もしない場合は、既存のポリシーから「**5 Year Delete**」を選択します (この演習では設定を適用するだけです)。
